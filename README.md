## Container Documentation for Aspnet-Runtime Documentation

The CleanStart Aspnet-Runtime image provides a production-ready, security-hardened container optimized for enterprise environments. Built on a minimal base OS with comprehensive security hardening, this image delivers reliable application execution with advanced security features.
...
docker run -d --name aspnet-runtime-prod \
  ...
  ghcr.io/cleanstart-containers/aspnet-runtime:latest
```

Volume Mount Mount local directory for persistent data

```bash
docker run -v /app:/app ghcr.io/cleanstart-containers/aspnet-runtime:latest
```

Port Forwarding Run with custom port mappings

```bash
docker run -p 8080:8080 ghcr.io/cleanstart-containers/aspnet-runtime:latest
```
